...
def use_eof_to_calculate_pc(var,data_history):
...
def use_pc_to_predict_data(var,data_history,days_history,days_predict,target_date):
    var_pc,var_eof,var_res,mask,var_cli = use_eof_to_calculate_pc(var,data_history)
    ...
    for k_pc in range(1,days_predict+1):
        model = load_model(f'./finalmodel-{var}a/PC{k_pc}.h5')
...
def writeNC(data_out_file, long, latg, date_predict, sst_predict, adt_predict): 
    ...
    # sst_predict = sst_predict.astype(np.float32)
    # adt_predict = adt_predict.astype(np.float32)
    f_w.variables['sst'][:]  = sst_predict
    f_w.variables['adt'][:]  = adt_predict
    
    f_w.close
                    
def funciton_4_main_cycle(var, data_in_path, target_datedays_history) 
...
if __name__=='__main__':
    ...
    data_out_path = f'./result/{target_date.strftime("%Y")}'
    ...
    writeNC(data_out_file, long, latg, date_predict, sst_predict, adt_predict)
    # writeNC(data_out_file, long,latg, data_predict, data_ano_predict)